...
\abstract{The NANOGrav 15-year pulsar timing array data set provides strong evidence for a nanohertz common process with Hellings--Downs spatial correlations. We use this result as the angular-correlation input and ask a narrower source-physics question: does the public Hellings--Downs free-spectrum product contain Bayesian evidence for spectral curvature beyond a strict, scale-free SMBHB power law? We construct a reproducible compressed likelihood from the 30-frequency free-spectrum KDE product and compare four source spectra under the same HD-correlated stochastic-background assumption: a fixed-slope SMBHB power law, an SMBHB environmental-turnover model, a first-order phase-transition template defined in OmegaGW(f) and converted to characteristic strain, and a simple cosmic-string effective-slope proxy. This separation gives the analysis a natural symmetry structure: the angular sector tests the rotationally invariant tensor-correlation signature of an approximately isotropic background, while the spectral sector tests which physical scale or symmetry-breaking mechanism best explains departures from a scale-free spectrum. Relative to a fixed gamma=13/3 SMBHB power law, the compressed spectral evidence favors curvature, with Delta ln Z = 3.90 +/- 0.02 for the environmental SMBHB model and Delta ln Z = 4.90 +/- 0.03 for the phase-transition template; the cosmic-string proxy is weak, with Delta ln Z = 0.44 +/- 0.02. We therefore find firm evidence for spectral curvature within the public KDE compression. The highest compressed evidence belongs to the phase-transition template, making it the strongest high-impact cosmological competitor in this model set. The SMBHB environmental-turnover interpretation remains the leading conservative astrophysical explanation because it accounts for most of the curvature with known binary-hardening physics and maps the bend frequency to plausible parsec-scale nuclear densities. These results are compressed spectral evidences, not full timing-likelihood evidences, and the accompanying release provides the scripts, priors, posterior samples, and figure-generation workflow needed to reproduce them.}
