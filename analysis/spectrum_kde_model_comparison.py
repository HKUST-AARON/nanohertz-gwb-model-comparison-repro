...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--kde-dir",
        type=Path,
        default=Path("data_sources/NANOGrav15yr_KDE-FreeSpectra_v1.1.0/ceffyl_data/30f_fs{hd}_ceffyl"),
    )
    parser.add_argument(
        "--outdir",
        type=Path,
...
def make_priors(overrides: Dict[str, Tuple[float, float]] | None = None):
    ...
    def pl_free(rng: np.random.Generator, n: int):
        a = rng.uniform(-17.0, -14.0, size=n)
        gamma = rng.uniform(0.0, 7.0, size=n)
        return np.column_stack([a, gamma]), ("log10_A", "gamma")
    ...
    return pl_fixed, pl_free, sbpl, cosmic_strings, phase_transition


def models_from_priors(priors):
    pl_fixed, pl_free, sbpl, cosmic_strings, phase_transition = priors
    return {
        "smbhb_pl": (
            pl_fixed,
        ...
        "smbhb_pl_free": (
            pl_free,
            lambda freqs, p: log10_rho_powerlaw(freqs, p[:, 0], p[:, 1]),
        ),
        "smbhb_env": (
            sbpl,
...
def run_kde_product_robustness(args: argparse.Namespace, outdir: Path) -> None:
    kde_root = args.kde_dir.parent
    product_dirs = {
        "HD only": kde_root / "30f_fs{hd}_ceffyl",
        "HD+MP+DP, HD component": kde_root / "30f_fs{hd+mp+dp}_ceffyl_hd-only",
        "HD+MP+DP+CP, HD component": kde_root / "30f_fs{hd+mp+dp+cp}_ceffyl_hd-only",
    }
    results = {}
